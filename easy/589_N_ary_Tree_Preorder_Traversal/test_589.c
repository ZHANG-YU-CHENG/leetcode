...
extern int* preorder(struct Node* root, int* returnSize);

TEST(preorderTest, test1) {
    //level 0 (root)
    ...
    int expect[] = {1,3,5,6,2,4};
    for(int i=0; i<returnSize; ++i)
        EXPECT_EQ(ans[i], expect[i]);
}

TEST(preorderTest, test2) {
    //level 0 (root)
    struct Node* root = (struct Node*)malloc(sizeof(struct Node));
    root->val = 1;
    root->numChildren = 4;
    ...
    int expect[] = {1,2,3,6,7,11,14,4,8,12,5,9,13,10};
    for(int i=0; i<returnSize; ++i)
        EXPECT_EQ(ans[i], expect[i]);
}
